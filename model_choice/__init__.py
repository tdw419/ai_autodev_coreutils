...
def _stream_wrapper(
    ...
    Only caches if the stream completes without error. Partial responses
    from interrupted streams are NOT cached (they'd be corrupt).
    """
    collected = []
    stream_error = None
    try:
        ...
        # Consumer stopped early -- don't cache partial response
        return
    finally:
        # Only cache complete, successful responses
        if stream_error is None and use_cache and collected:
...
def _stream_with_fallback(
    ...
    Enhanced:
    - Rate-limits the primary provider
    - Only caches complete, error-free responses
    - Tracks failures for providers that error mid-stream
    """
    from .fallback import _build_fallback_chain

    errors = []

    # Rate-limit the primary provider
    limiter_ctx = _rate_limit(start_provider)
    rate_limited = False
    try:
        ...
        rate_limited = True  # we got past the connection phase
    except Exception as e:
        # Release rate limit on connection failure
        try:
            limiter_ctx.__exit__(None, None, None)
        except Exception:
            pass
        ...
        _tracker.record(start_provider.provider, success=False)
        ...
        for fb_provider in chain:
            try:
                ...
                _tracker.record(fb_provider.provider, success=False)
                continue
        else:
            raise RuntimeError(
    ...
    try:
        ...
        # Consumer stopped early -- don't cache, don't track error
        pass
    finally:
        # Release rate limit
        if rate_limited:
...
def ollama_stop() -> bool:
    """Stop ollama. Returns True if confirmed stopped."""
    from .ollama import stop_ollama
    return stop_ollama()


def ollama_lifecycle() -> dict:
    """Get detailed lifecycle state of ollama."""
    from .ollama import lifecycle_state
    return lifecycle_state()


def ollama_cleanup() -> dict:
    """Clean up orphaned ollama resources (stale processes, temp files)."""
    from .ollama import cleanup
    return cleanup()


def ollama_shutdown() -> dict:
    """Full shutdown: stop ollama and cleanup resources."""
    from .ollama import shutdown
    return shutdown()


def ollama_init(model: str | None = None) -> dict:
    """Initialize ollama: cleanup, start, load model."""
    from .ollama import init
