...
### 10. `autodev-decide` -- Pick between options using evidence

Takes a question, discrete options, and evidence. Returns a structured
decision with confidence score and reasoning. Use this whenever an agent
needs to make a judgment call -- which task to prioritize, whether to
keep or revert, whether to escalate.

```bash
# Decide which task to work on next
autodev-decide "Which task first?" \
  --options task_a task_b task_c \
  --evidence .autodev/tasks/task_000.md .autodev/tasks/task_001.md

# Pipe evidence from git
git diff HEAD~1 | autodev-decide "Keep these changes?" --options keep revert

# Decide with criteria
autodev-decide "Ship it?" --options yes no \
  --evidence test_output.log \
  --criteria "all tests pass, no regressions, docs updated"

# JSON output for piping
autodev-decide "Next step?" --options explore plan implement --json

# Don't log the decision
autodev-decide "Quick check?" --options a b --no-log
```

Output: Text with choice, confidence %, and reasoning. `--json` gives
`{choice, confidence, reasoning, matched_option}`. All decisions are
appended to `.autodev/decisions.jsonl` (audit trail).

Exit code: 0 if confidence >= 30%, 1 if below (low-confidence signal).

### 11. `autodev-discover` -- Find new work

Scans three sources to discover what needs doing:
- `~/zion/projects/` -- dirty git, incomplete roadmaps, stale work
- RAG knowledge base -- pending ideas, unfinished features, known gaps
- Geometry OS -- roadmap completeness, test failures, open issues

Returns a prioritized list of work items. Can feed into the roadmap/flow pipeline.
...
autodev-discover --source projects
autodev-discover --source rag
autodev-discover --source geo

# Write top findings into roadmap
autodev-discover --update-roadmap

# Run full flow on the best finding
autodev-discover --into-flow

# Limit results
autodev-discover --limit 10
```
...
### Pattern 2: Snapshot, work, verify, decide
...
# Let AI decide: keep or revert?
git diff HEAD~1 | autodev-decide "Keep these changes?" \
  --options keep revert \
  --criteria "tests pass, no regressions, code is clean"

# If it decided to revert
autodev-snapshot --restore before-refactor
```
...
### Pattern 4: Discover and execute

```bash
# Find what needs doing
autodev-discover --json > findings.json

# Pick the best project to work on
autodev-decide "Which project to focus on?" \
  --options geometry_os pixelpack autodev_coreutils \
  --evidence findings.json \
  --criteria "impact, freshness, tractability"

# Run full flow on the chosen project
autodev-discover --into-flow

# Or manually: discover -> roadmap -> work
autodev-discover --source geo --update-roadmap
autodev-task-split .autodev/roadmap.yaml -n 3
rfl run --from-file .autodev/tasks/seed_task_000.txt -w ~/zion/projects/geometry_os/geometry_os
```
...
.  possibility_tree.json     # raw exploration tree from possibilities
  ...
  decisions.jsonl           # append-only audit trail from autodev-decide
...
10. **Use `autodev-decide` for judgment calls.** Whenever you need to pick
    between options (which task, keep/revert, escalate/continue), pipe the
    evidence in and let it decide. It logs everything to `decisions.jsonl`
